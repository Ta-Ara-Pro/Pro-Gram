...
const PostStats = ({ post, userId}: PostStatsProps) => {
  ...
  const handleLikePost = (e: React.MouseEvent) => {
    ...
    // handel saving post
  const handelSavePost = (e: React.MouseEvent) => { e.stopPropagation();

    if ( savedPostRecord ) { setIsSaved(false);  deleteSavedPost(savedPostRecord.$id);
      } else {  savePost({ postId: post?.$id || '', userId })
     setIsSaved(true);
    }
  }
